...
def logistic_regression(inp, X_train, X_test, y_train, y_test, arr, sensitive_param = None, dataset_name = "", save_model=False):
    # print(inp)
    arrs, features = xml_parser.xml_parser('logistic_regression_Params.xml',inp)
    # print(arr)
    write_file = None
    try:
        ...
        if save_model:
            write_file = f"./updated_trained_models/logisticRegression_{dataset_name}_{sensitive_param}_{arr[0]}_{arr[1]}_{arr[2]}_{arr[3]}_{arr[4]}_{arr[5]}_{arr[6]}_{arr[7]}"\
